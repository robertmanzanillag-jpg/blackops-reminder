...
async function main() {
  ...
  const metricoolQueuedRows = Number(metricool100Handoff?.totals?.rows || 0);
  const expectedBatchCount = Math.ceil(metricoolQueuedRows / 10);
  ...
  const metricool100 = {
    ...
    weeklyGapRows: Math.max(0, 100 - metricoolQueuedRows),
