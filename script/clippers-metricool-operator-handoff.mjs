...
function rollForwardPendingSchedules(
  rows,
  existingEvidenceById = new Map(),
  now = new Date(),
  minLeadMinutes = Number(process.env.CLIPPERS_ROLL_FORWARD_MIN_LEAD_MINUTES || 20),
) {
...
async function main() {
  ...
  const rollForwardLeadRaw = process.env.CLIPPERS_ROLL_FORWARD_MIN_LEAD_MINUTES;
  const rollForwardRequested = Object.hasOwn(process.env, "CLIPPERS_ROLL_FORWARD_MIN_LEAD_MINUTES");
  const rollForwardLeadMinutes = Number(rollForwardLeadRaw);
  if (rollForwardRequested && (!Number.isFinite(rollForwardLeadMinutes) || rollForwardLeadMinutes < 1 || rollForwardLeadMinutes > 10_080)) {
    throw new Error("Metricool handoff blocked: CLIPPERS_ROLL_FORWARD_MIN_LEAD_MINUTES must be a number from 1 to 10080.");
  }
  const approvalStatusAllowsLocalRollForward = rollForwardRequested && approvalRun.status === "needs_review";
  if (approvalRun.status !== "ready_for_operator" && !approvalStatusAllowsLocalRollForward) {
    throw new Error(`Metricool handoff blocked: approval run status is ${approvalRun.status || "missing"}.`);
  }
  const expectedReviewRows = Number(
    approvalRun.totals?.metricoolQueuedForApproval
      ?? approvalRun.totals?.approvalSessionReadyForReview
      ?? approvalRun.totals?.draftReady
      ?? 0,
  );
  if (!Number.isInteger(expectedReviewRows) || expectedReviewRows < 1 || expectedReviewRows > Number(approvalRun.totals?.productionItems || 100)) {
    throw new Error(`Metricool handoff blocked: invalid expected review row count ${expectedReviewRows}.`);
  }
  if (rows.length !== expectedReviewRows) {
    throw new Error(`Metricool handoff blocked: expected ${expectedReviewRows} review-ready run sheet rows, found ${rows.length}.`);
  }
