...
const PORT = "55436";
const FORBIDDEN_ENV = /^(?:HEYGEN(?:_|$)|AWS(?:_|$)|AI_MEDIA_STUDIO_(?:HEYGEN|ASSET)_(?:ACCESS_KEY_ID|SECRET_ACCESS_KEY|SESSION_TOKEN|BUCKET|ENDPOINT))/u;
...
async function waitUntilReady(socketDirectory: string, postgres: ChildProcess): Promise<void> {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    try {
      await run("pg_isready", ["-q", "-h", socketDirectory, "-p", PORT, "-U", "postgres"]);
      return;
    } catch {
...
async function stop(dataDirectory: string, postgres: ChildProcess | undefined): Promise<void> {
  ...
  try {
    await run("pg_ctl", ["-D", dataDirectory, "-m", "immediate", "-w", "stop"]);
  } catch {
...
async function main(): Promise<void> {
  const forbidden = Object.entries(process.env)
    .filter(([name, value]) => Boolean(value?.trim()) && FORBIDDEN_ENV.test(name))
    .map(([name]) => name)
    .sort();
  if (process.env.DATABASE_URL?.trim() || process.env.TEST_DATABASE_URL?.trim() || forbidden.length) {
    throw new Error(`offline one-slot harness refuses database/provider/cloud environment (${forbidden.join(",") || "DATABASE_URL"})`);
  }
  ...
  try {
    ...
    const safeEnvironment = Object.fromEntries(Object.entries(process.env).filter(([name]) =>
      name !== "DATABASE_URL" && name !== "TEST_DATABASE_URL" && !FORBIDDEN_ENV.test(name)));
    testStarted = true;
    await run("node", ["--import", "tsx", "--test", "--test-concurrency=1",
      "tests/ai-media-studio-offline-one-slot-runtime.test.ts"], { ...safeEnvironment, TEST_DATABASE_URL: testUrl });
  } catch (error) {
