...
async function main(): Promise<void> {
  if (process.env.DATABASE_URL?.trim() || process.env.TEST_DATABASE_URL?.trim()) {
    throw new Error("reusable-script harness refuses DATABASE_URL and external TEST_DATABASE_URL");
  }
  ...
  try {
    ...
    const {
      ...
      TEST_DATABASE_URL: _external,
      AI_MEDIA_STUDIO_SECRET_HEYGEN_API_KEY: _secureHeyGenKey,
      HEYGEN_API_KEY: _legacyHeyGenKey,
      ...safeEnvironment
    } = process.env;
    testStarted = true;
    await run("node", ["--import", "tsx", "--test", "--test-concurrency=1",
      "tests/ai-media-studio-reusable-script-assets-pg.test.ts"],
    { ...safeEnvironment, TEST_DATABASE_URL: testUrl });
  } catch (error) {
