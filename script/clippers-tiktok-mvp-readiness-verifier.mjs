...
async function main() {
  ...
  const queuedRows = Number(approvalRun.totals?.metricoolQueuedForApproval || 0);
  const expectedBatchCount = Math.ceil(queuedRows / 10);
  ...
  const checks = [
    ...
    check(
      "active_tiktok_rows_match",
      "Queued rows are TikTok-only and match the active run",
      queuedRows > 0
        && Number(approvalSession.totals?.tiktok || 0) === queuedRows
      ...
      "active_queue_mismatch_or_non_tiktok_rows",
      "Regenerate the active Metricool approval run with only TikTok-ready accounts; track any gap to 100 separately.",
    ),
    check(
      "operator_handoff_batches_ready",
      "All active Metricool batches are ready for operator review",
      operatorHandoff.status === "ready_for_operator"
        && Number(operatorHandoff.totals?.rows || 0) === queuedRows
        && (operatorHandoff.batches || []).length === expectedBatchCount
    ...
    check(
      ...
      launchControl.status === "ready_for_metricool_review"
        && Number(launchControl.totals?.rows || 0) === queuedRows
        && Number(launchControl.totals?.tiktok || 0) === queuedRows
  ...
  const summary = {
    ...
    active: {
      ...
      currentBatchId,
      totalRows: queuedRows,
      weeklyTargetRows: 100,
      weeklyGapRows: Math.max(0, 100 - queuedRows),
