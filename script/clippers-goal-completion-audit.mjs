...
async function main() {
  ...
  const approvalOnlyQueueReady = ["ready_for_operator", "needs_review"].includes(approvalRun.status)
    && queuedRows > 0
  ...
  const strictTikTokSession = queuedRows > 0
  ...
  const approvalRunPublishMode = approvalRunRealPublishEnabled || Number(approvalRun.totals?.readyToSend || 0) > 0
    ? "unsafe_or_direct"
    : "approval_required";
  const expectedPublishedRows = queuedRows;
