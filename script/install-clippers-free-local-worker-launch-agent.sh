...
mkdir -p "$AGENT_DIR" "$LOG_DIR" "$CONTENT_LOG_DIR" "$WATCHDOG_LOG_DIR"
...
<plist version="1.0"><dict>
  ...
  <key>ProgramArguments</key><array><string>$(escape_xml "$NODE_PATH")</string><string>$(escape_xml "$script_path")</string>${extra_program_argument_xml}</array>
...
if [[ "$DRY_RUN" == "true" ]]; then
  echo "Clippers LaunchAgents validated without installation: TikTok worker at ${WORKER_HOUR}:${WORKER_MINUTE}, content worker at ${CONTENT_WORKER_HOUR}:${CONTENT_WORKER_MINUTE}, watchdog at ${WATCHDOG_HOUR}:${WATCHDOG_MINUTE} (${TIME_ZONE}); runtime $RUNTIME_ROOT"
  exit 0
fi

DOMAIN="gui/$(id -u)"
for specification in "$WORKER_LABEL:$WORKER_PLIST" "$CONTENT_WORKER_LABEL:$CONTENT_WORKER_PLIST" "$WATCHDOG_LABEL:$WATCHDOG_PLIST"; do
  label="${specification%%:*}"
  plist="${specification#*:}"
...
echo "Clippers LaunchAgents installed and verified: TikTok worker daily at ${WORKER_HOUR}:${WORKER_MINUTE}, content worker daily at ${CONTENT_WORKER_HOUR}:${CONTENT_WORKER_MINUTE}, watchdog daily at ${WATCHDOG_HOUR}:${WATCHDOG_MINUTE} (${TIME_ZONE}); runtime $RUNTIME_ROOT"
