...
async function main(): Promise<void> {
  if (process.env.DATABASE_URL?.trim() || process.env.TEST_DATABASE_URL?.trim()) {
    throw new Error("offline preflight harness refuses DATABASE_URL and external TEST_DATABASE_URL");
  }
  ...
  try {
    ...
    await run("node", ["--import", "tsx", "--test", "--test-concurrency=1",
      "tests/ai-media-studio-offline-launch-preflight-postgres.test.ts"], { ...safeEnvironment, TEST_DATABASE_URL: testUrl });
  } catch (error) {
