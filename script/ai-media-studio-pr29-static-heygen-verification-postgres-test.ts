...
async function main(): Promise<void> {
  if (process.env.DATABASE_URL?.trim() || process.env.TEST_DATABASE_URL?.trim()) {
    throw new Error("PR29 harness refuses DATABASE_URL and external TEST_DATABASE_URL");
  }
  ...
  try {
    ...
    const {
      DATABASE_URL: _database, TEST_DATABASE_URL: _external,
      AI_MEDIA_STUDIO_SECRET_HEYGEN_API_KEY: _heyGenKey, HEYGEN_API_KEY: _legacyHeyGenKey,
      ...safeEnvironment
    } = process.env;
    testStarted = true;
