...
let materializedCookiePath: string | null = null;
...
function configuredYoutubeCookieArgs(): string[] {
  ...
  const base64CookiesPath = materializeConfiguredYoutubeCookies();
  if (base64CookiesPath) return ["--cookies", base64CookiesPath];
...
function collectBase64CookieChunks(): string | null {
  const direct = process.env.YT_DLP_COOKIES_B64?.trim();
  if (hasConfiguredValue(direct)) return direct;

  const chunks: string[] = [];
  for (let index = 1; index <= 50; index += 1) {
    const chunk = process.env[`YT_DLP_COOKIES_B64_${index}`]?.trim();
    if (!chunk) break;
    chunks.push(chunk);
  }
...
function materializeConfiguredYoutubeCookies(): string | null {
  if (materializedCookiePath) return materializedCookiePath;

  const encoded = collectBase64CookieChunks();
  ...
  try {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yt-dlp-cookies-"));
    const cookiePath = path.join(dir, "cookies.txt");
    fs.writeFileSync(cookiePath, decoded.endsWith("\n") ? decoded : `${decoded}\n`, { mode: 0o600 });
    materializedCookiePath = cookiePath;
    return cookiePath;
  } catch (error) {
    console.warn("[youtube-downloader] Could not materialize YT_DLP_COOKIES_B64 cookies:", error instanceof Error ? error.message : error);
    return null;
  }
}
...
export function buildYtDlpCommandSpecs(params: {
  ...
  const format = params.format || (params.mode === "video"
...
export function formatYtDlpFailureMessage(rawError: string, mediaLabel: "video" | "audio" = "video"): string {
  ...
  if (/sign in to confirm|not a bot|confirm you.?re not a bot|use --cookies|cookies for the authentication|http error 429/.test(lower)) {
    return [
      ...
      "Para automatizarlo de forma estable, configura cookies de YouTube en YT_DLP_COOKIES_B64 o en YT_DLP_COOKIES_B64_1/2/3...; si YouTube sigue bloqueando, sube el MP4 fuente a Google Drive y pásame ese archivo como entrada.",
    ].join(" ");
  }

  if (/this video is unavailable|video unavailable|private video|video has been removed|not available/.test(lower)) {
    return [
      `No pude descargar el ${mediaLabel} de YouTube porque YouTube lo reportó como no disponible para yt-dlp.`,
      "Verifica que el link abra desde el mismo Chrome de la Mac o usa un MP4 fuente ya descargado/subido a Drive.",
    ].join(" ");
  }
