...
function revenueMoneySprintReviewDigest(value: string) {
  return createHash("sha256").update(value, "utf8").digest("hex");
}

function isRevenueMoneySprintReviewEvidenceRef(value: unknown) {
  if (typeof value !== "string") return false;
  const trimmed = value.trim();
  const normalized = trimmed.toLowerCase();
  if (normalized.length < 6) return false;
  if (/(^|[^a-z0-9])(ref|placeholder|replace|todo|tbd|example|sample|dummy)([^a-z0-9]|$)/i.test(normalized)) return false;
  const absoluteRevenueWorkspace = path.resolve(process.cwd(), "revenue_workspace");
  const resolved = trimmed.startsWith("revenue_workspace/")
    ? path.resolve(process.cwd(), trimmed)
    : path.isAbsolute(trimmed)
      ? path.resolve(trimmed)
      : "";
  if (!resolved || !resolved.startsWith(`${absoluteRevenueWorkspace}${path.sep}`)) return false;
  if (!resolved.includes(`${path.sep}approval-packets${path.sep}`)
    && !resolved.includes(`${path.sep}public-verification${path.sep}`)) return false;
  try {
    const realWorkspace = realpathSync(absoluteRevenueWorkspace);
    if (lstatSync(resolved).isSymbolicLink()) return false;
    const realResolved = realpathSync(resolved);
    if (!realResolved.startsWith(`${realWorkspace}${path.sep}`)) return false;
    if (!realResolved.includes(`${path.sep}approval-packets${path.sep}`)
      && !realResolved.includes(`${path.sep}public-verification${path.sep}`)) return false;
    return statSync(realResolved).isFile();
  } catch {
    return false;
  }
}

function readRevenueMoneySprintReviewEvidence(value: unknown) {
  ...
  const resolved = trimmed.startsWith("revenue_workspace/")
    ...
    : path.isAbsolute(trimmed)
      ? path.resolve(trimmed)
      : "";
  if (!resolved || !resolved.startsWith(`${absoluteRevenueWorkspace}${path.sep}`)) return "";
  try {
    const realWorkspace = realpathSync(absoluteRevenueWorkspace);
    if (lstatSync(resolved).isSymbolicLink()) return "";
    const realResolved = realpathSync(resolved);
    if (!realResolved.startsWith(`${realWorkspace}${path.sep}`)) return "";
    if (!realResolved.includes(`${path.sep}approval-packets${path.sep}`)
      && !realResolved.includes(`${path.sep}public-verification${path.sep}`)) return "";
    if (!statSync(realResolved).isFile()) return "";
...
export function validateRevenueMoneySprintRunPacketReview(review: MoneySprintRunPacketReviewJson): {
  ...
  const rawRequestBody = packet?.requestBody && typeof packet.requestBody === "object"
    ? packet.requestBody as Record<string, unknown>
    : {};
  const reviewEvidenceContent = readRevenueMoneySprintReviewEvidence(rawRequestBody.reviewPacketEvidence);
  const expectedDigest = requestBody ? revenueMoneySprintReviewDigest(requestBody.seedLeadBatchText) : "";
  ...
  const errors = [
    ...
    rawRequestBody.approvedByRobert !== false && "requestBody.approvedByRobert must remain false until --execute --approved-by-robert.",
    !isRevenueMoneySprintReviewEvidenceRef(rawRequestBody.reviewPacketEvidence) && "requestBody.reviewPacketEvidence must be an existing revenue_workspace approval/public-verification file.",
    !reviewEvidenceContent.includes("Approval status: approved_by_robert") && "requestBody.reviewPacketEvidence must include Approval status: approved_by_robert.",
    (typeof rawRequestBody.reviewPacketEvidenceDigest !== "string"
      || !/^[a-f0-9]{64}$/.test(rawRequestBody.reviewPacketEvidenceDigest)
      || rawRequestBody.reviewPacketEvidenceDigest !== expectedDigest
      || !reviewEvidenceContent.includes(rawRequestBody.reviewPacketEvidenceDigest)) && "requestBody.reviewPacketEvidenceDigest must be present in the review evidence file.",
