...
let remoteMutationTail = Promise.resolve();

function mutateRemoteControl(mutation: (state: BlackRoomRemoteControlState) => void): Promise<BlackRoomRemoteControlState> {
  const next = remoteMutationTail.then(async () => {
    const state = await readBlackRoomRemoteControl();
    mutation(state);
    await writeBlackRoomRemoteControl(state);
    return state;
  });
  remoteMutationTail = next.then(() => undefined, () => undefined);
  return next;
}
...
const ids=['queued','processing','retry','scheduled','completed'],durations=[15,30,60,120,300,600],labels={15:'15s',30:'30s',60:'60s',120:'2m',300:'5m',600:'10m'};let busy=false;async function req(path,opt){const r=await fetch(path,opt),d=await r.json();if(!r.ok){if(r.status===401)throw new Error('Inicia sesión en BlackOps para controlar el agente.');throw new Error(d.error||'Error')}return d}function render(d){const s=d.agent,remote=d.remote||{},desired=Boolean(remote.desiredEnabled),working=desired&&remote.online&&s.enabled;badge.textContent=working?'Trabajando':desired?'En cola':'Pausado';badge.className='badge'+(working?' on':'');play.hidden=desired;pause.hidden=!desired;weeks.disabled=desired;ids.forEach(id=>document.getElementById(id).textContent=s.totals[id]||0);device.textContent=remote.online?'● Mac conectada · orden sincronizada':'○ Mac desconectada · la orden quedará en cola hasta que se encienda';experiment.textContent=durations.map(x=>labels[x]+': '+(s.durationSamples[x]||0)).join(' · ')+' · '+s.usedSourceVideos+' videos únicos';next.textContent=s.nextJob?'Próximo lote: '+s.nextJob.targetDate+' · '+s.nextJob.status:'Pulsa Iniciar agente para comenzar.'}async function run(path,opt){if(busy)return;busy=true;try{render(await req(path,opt))}catch(e){next.textContent=e.message}finally{busy=false}}play.onclick=()=>run('/api/blackroom-agent/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({weeks:Number(weeks.value)})});pause.onclick=()=>run('/api/blackroom-agent/pause',{method:'POST'});refresh.onclick=()=>run('/api/blackroom-agent');run('/api/blackroom-agent');setInterval(()=>run('/api/blackroom-agent'),15000);
</script></body></html>`;
...
export function registerBlackRoomControlRoutes(app: Express): void {
  ...
  app.post("/api/blackroom-agent/start", async (req, res) => {
    try {
      ...
      const remote = await mutateRemoteControl((current) => setBlackRoomRemoteCommand(current, true, Number(req.body?.weeks || 2)));
  ...
  app.post("/api/blackroom-agent/pause", async (_req, res) => {
    try {
      ...
      const remote = await mutateRemoteControl((current) => setBlackRoomRemoteCommand(current, false));
      res.json({ agent: summarizeBlackRoomQueue(state), remote: remoteView(remote) });
  ...
  app.post("/api/blackroom-agent/remote", async (req, res) => {
    if (!hasValidBlackRoomRemoteToken(req.get("authorization"))) return res.status(401).json({ error: "Invalid BlackRoom device token" });
    try {
      const remote = await mutateRemoteControl((current) => recordBlackRoomRemoteHeartbeat(current, {
