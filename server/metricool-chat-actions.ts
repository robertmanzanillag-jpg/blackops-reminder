...
export function sanitizeMetricoolAutomationInput(input: unknown): MetricoolAutomationInput {
  ...
  const publishMode = value.publishMode === "draft_only" || value.publishMode === "approval_required"
    ? value.publishMode
    : "approval_required";
...
export function buildDirectMetricoolCommand(message?: string): { content: string; command: string } | null {
  ...
  const requestedAuto = /\b(auto|automatico|automatica|automaticamente|sin aprobar|live|publica ya|postea ya)\b/.test(text);
  ...
  return {
    content: requestedAuto
      ? "Dale. Voy a preparar la cola Metricool en approval_required. Aunque pediste automatico/live, el MVP queda bloqueado por revision manual en Metricool y no publica sin evidencia real."
