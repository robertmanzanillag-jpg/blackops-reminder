...
export const FAKE_VIDEO_PROVIDER_ACCOUNT_ID = "00000000-0000-4000-8000-0000000000f0";

export class FakeVideoProvider implements VideoProvider {
  readonly key = "fake";
  readonly providerAccountId: string;
  constructor(private readonly options: { autoComplete?: boolean; providerAccountId?: string } = { autoComplete: true }) {
    this.providerAccountId = options.providerAccountId?.trim() || FAKE_VIDEO_PROVIDER_ACCOUNT_ID;
  }
  ...
  async submit(request: Parameters<VideoProvider["submit"]>[0], context: Parameters<VideoProvider["submit"]>[1]) {
    if (context.providerAccountId !== this.providerAccountId) throw new Error("Provider account scope mismatch");
  ...
  parseWebhook(payload: unknown, context: Parameters<VideoProvider["parseWebhook"]>[1]): ProviderWebhookEvent {
    if (!context.providerAccountId.trim()) throw new Error("Provider account scope is required");
    ...
    const eventId = text(input.event_id) ?? text(input.id) ?? text(context.fallbackEventId);
    ...
    return { eventId, providerKey: this.key, providerAccountId: context.providerAccountId, providerJobId, status, occurredAt: new Date(occurredAt).toISOString(),
