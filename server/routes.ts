...
import { buildRevenueFirstMoneyCommandCenter } from "./revenue-first-money-command-center-cli";
...
<body>
  ...
  app.get("/api/revenue-engine/first-money-command-center", async (_req, res) => {
    try {
      res.json(buildRevenueFirstMoneyCommandCenter({ mode: "first-sprint", json: false }));
    } catch (error) {
