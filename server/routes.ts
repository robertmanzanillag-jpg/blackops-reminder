...
<body>
  ...
  app.use("/api/revenue-engine", (req, res, next) => {
    acquireRevenueEngineScope(getCurrentUserId(req)).then((releaseRoute) => {
      let released = false;
      const release = () => {
        if (released) return;
        released = true;
  ...
  app.get("/api/revenue-engine/stripe/status", async (_req, res) => {
    try {
      res.json(await getRevenueStripeStatus());
    } catch {
      res.status(500).json({ error: "Failed to verify Robert Websites Stripe status" });
    }
  });

  app.post("/api/revenue-engine/stripe/checkout", async (req, res) => {
    try {
      const input = revenueStripeCheckoutSchema.parse(req.body);
      res.status(201).json(await createRevenueStripeCheckout(input, {
        revenueUserId: getCurrentUserId(req),
      }));
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ error: error.errors });
      ...
      res.status(502).json({
        error: error instanceof Error ? error.message : "Failed to create Robert Websites Checkout Session",
      });
    }
  });
