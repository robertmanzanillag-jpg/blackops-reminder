...
function reviewEvidenceDigest(value: string) {
  return createHash("sha256").update(value, "utf8").digest("hex");
}

function writeReviewEvidenceFixture(batchText: string, slug = "run-packet") {
  const relativePath = `public-verification/${slug}.md`;
  const filePath = path.join(process.cwd(), "revenue_workspace", relativePath);
  const digest = reviewEvidenceDigest(batchText);
  mkdirSync(path.dirname(filePath), { recursive: true });
  writeFileSync(filePath, `# Reviewed public candidate packet\n\nApproval status: approved_by_robert\nBatch digest: ${digest}\n`, "utf8");
  return { ref: `revenue_workspace/${relativePath}`, digest };
}
...
function reviewPacket(overrides: Record<string, unknown> = {}) {
  ...
  const reviewEvidence = writeReviewEvidenceFixture(seedLeadBatchText);
  return {
    ...
    moneySprintRunPacket: {
      ...
      requestBody: {
        ...
        reviewPacketEvidence: reviewEvidence.ref,
        reviewPacketEvidenceDigest: reviewEvidence.digest,
        seedLeads: [],
        seedLeadBatchText,
      },
      safety: {
...
test("blocks tampered packet when digest does not match the reviewed batch", () => {
  const base = reviewPacket();
  const tampered = reviewPacket({
    moneySprintRunPacket: {
      ...base.moneySprintRunPacket,
      requestBody: {
        ...
        seedLeadBatchText: [
          "business|area|niche|website|channel|contact|sourceUrl|recipientEmail|evidence|painPoint|offer|contactName|summary",
          "Digest Swap Cafe|Miami|coffee shop|no_website|email|owner@digestswap.example|https://example.com/digest-swap|owner@digestswap.example|Public listing has no website, recent menu photos and a visible public owner email.|Needs online menu capture and follow-up.|3600|Owner|Digest Swap Cafe has no dedicated website and a clear catering opportunity.",
        ].join("\n"),
      },
    },
  });
  ...
  assert.match(validation.errors.join("; "), /reviewPacketEvidenceDigest must be present/);
});
