...
import type { PrepareProductionBatchInput, ProductionBatchRepository } from "../server/ai-media-studio/production-batches/contracts";
  ...
  return {
    ...
    groups: Array.from({ length: 5 }, (_, member) => ({ memberId: key("member", member + 10), creatorName: `Creator ${member + 1}`,
      items: Array.from({ length: 10 }, (_, video) => ({ slotId: key("slot", 100 + member * 10 + video), videoNumber: video + 1,
        ...
        script: { key: key("script", 200 + member * 10 + video), title: `Script ${video + 1}`, status: "draft" as const, variantCount: 3 },
      })) })),
  };
}
