...
test("prepares exactly ten deterministic approval-only MAC Energy drafts", async () => {
  ...
  assert.equal(manifest.clips.length, 10);
  ...
  assert.deepEqual(manifest.clips.map((clip) => clip.sourceName), [
    ...
    "fullsend_mma_1783551884_3937040527681003529_53903826786.mp4",
    "mmajunkie_1783653297_3937890821851367590_1249064182.mp4",
    "mmajunkie_1783653664_3937894387152381122_1249064182.mp4",
    "mmajunkie_1783654217_3937898989839153868_1249064182.mp4",
    "sportscenter_1783650464_3937867809743551946_505182045.mp4",
  ]);
  assert.equal(new Set(manifest.clips.map((clip) => clip.sourceName)).size, 10);
  ...
  assert.equal(csv.match(/approval_required/g)?.length, 10);
  assert.equal(csv.match(/false/g)?.length, 10);
  assert.match(approvalHtml, /Publishing is disabled/);
  assert.equal(approvalHtml.match(/approval_required/g)?.length, 10);

  const ffmpegCalls = (await readFile(item.ffmpegLog, "utf8")).trim().split("\n");
  assert.equal(ffmpegCalls.length, 10);
