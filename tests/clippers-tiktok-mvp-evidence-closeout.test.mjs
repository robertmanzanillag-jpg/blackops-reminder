...
test("TikTok MVP evidence closeout is wired into guarded API routes and UI controls", async () => {
  ...
  assert.match(page, /Preview clean; save still required/);
  assert.match(page, /Packet ready for preview; save still required/);
  assert.match(page, /Proof drop loaded for preview; save still required/);
  assert.doesNotMatch(page, /Proof links validos|Proof packet convertido|Proof drop importado/);
  ...
  assert.match(proofDropLoadHelper, /setTiktokMvpProofLinksPasteText\(cleanPacket\)/);
  assert.match(proofDropLoadHelper, /setTiktokMvpProofLinksPastePreview\(null\)/);
  assert.match(proofDropLoadHelper, /setTiktokMvpProofLinksPreview\(null\)/);
  assert.match(proofDropLoadHelper, /setTiktokMvpProofLinksSaveReceipt\(null\)/);
  ...
  const proofHandoffLoadHelperEnd = page.indexOf("const appendCredentialBatchTemplate", proofHandoffLoadHelperStart);
  assert.ok(proofHandoffLoadHelperStart > 0);
  assert.ok(proofHandoffLoadHelperEnd > proofHandoffLoadHelperStart);
  const proofHandoffLoadHelper = page.slice(proofHandoffLoadHelperStart, proofHandoffLoadHelperEnd);
  assert.match(proofHandoffLoadHelper, /setTiktokMvpProofLinksPasteText\(cleanPacket\)/);
  assert.match(proofHandoffLoadHelper, /setTiktokMvpProofLinksPastePreview\(null\)/);
  assert.match(proofHandoffLoadHelper, /setTiktokMvpProofLinksPreview\(null\)/);
  assert.match(proofHandoffLoadHelper, /setTiktokMvpProofLinksSaveReceipt\(null\)/);
  assert.match(proofHandoffLoadHelper, /setTiktokMvpProofLinksPreviewGate\(null\)/);
  assert.match(proofHandoffLoadHelper, /markGoalCompletionProofLinksPreviewGateStale\(\)/);
  assert.doesNotMatch(proofHandoffLoadHelper, /mutate\(/);
  assert.doesNotMatch(proofHandoffLoadHelper, /fetch\(|invalidateQueries|runTikTokMvpOperatorButton|PreviewMutation|SaveMutation|ImportMutation|IngestMutation|ApplyMutation|\.mutate/i);
  ...
  assert.match(page, /tiktokMvpProofLinksDropStarter: data\.tiktokMvpProofLinksDropStarter/);
  assert.match(page, /tiktokMvpProofLinksDropStatus: data\.tiktokMvpProofLinksDropStatus/);
  assert.match(page, /tiktokMvpProofLinksDropStarter\?: ClipperTikTokMvpProofLinksDropStarterSummary/);
...
test("TikTok MVP proof drop starter route blocks secret-like existing drops without returning raw text", async () => {
  const previousDrop = await readFile(proofLinksFilledDropPath, "utf8").catch(() => null);
  const previousProofLinks = await readFile(proofLinksPath, "utf8").catch(() => null);
  const secretLikeDrop = [
    "# unsafe local operator proof drop",
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${address.port}/api/clippers/create-tiktok-mvp-proof-links-drop-starter`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "{}",
    });
    ...
    assert.match(body.error, /secret-like text/i);
    assert.equal(body.tiktokMvpProofLinksDropStarter.status, "blocked_secret_like_existing");
    assert.equal(body.tiktokMvpProofLinksDropStarter.scope, "tiktok_only_metricool_mvp");
    assert.equal(body.tiktokMvpProofLinksDropStarter.launchMode, "metricool_approval_required");
    assert.equal(body.tiktokMvpProofLinksDropStarter.directSocialApisRequired, false);
    assert.equal(body.tiktokMvpProofLinksDropStarter.realPublishEnabled, false);
    ...
    assert.equal(await readFile(proofLinksFilledDropPath, "utf8"), secretLikeDrop);
    assert.equal(await readFile(proofLinksPath, "utf8").catch(() => null), previousProofLinks);
  } finally {
    if (listening) {
      await new Promise((resolve, reject) => {
