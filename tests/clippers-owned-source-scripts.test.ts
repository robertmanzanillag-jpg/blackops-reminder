...
test("TikTok batch runbook prepares Metricool operator instructions without publishing", async () => {
  ...
  assert.match(page, /tiktokMetricoolProofGateOpen/);
  assert.match(page, /controlFieldsPresent === true/);
  assert.match(page, /requiredLanes\.includes\("sports-daily:tiktok"\)/);
  assert.match(page, /failedPreflightChecks/);
  assert.match(page, /missingRequiredReports/);
  assert.match(page, /preflightNotReady/);
  assert.match(page, /clippers-tiktok-runbook-proof-gate-lock/);
  assert.match(page, /Metricool scheduling controls are locked until proof gate is ready/);
  assert.match(page, /Locked until TikTok Metricool proof gate is ready/);
...
test("TikTok MVP go-live packet combines account closeout and batch runbook safely", async () => {
  ...
  assert.equal(output.proofGateStatus, "blocked_needs_real_metricool_tiktok_proof");
  ...
  assert.equal(packet.directSocialApisRequired, false);
  assert.equal(packet.realPublishEnabled, false);
  assert.equal(packet.proofGate.status, "blocked_needs_real_metricool_tiktok_proof");
  assert.deepEqual(packet.proofGate.requiredLanes, ["sports-daily:tiktok", "meme-radar:tiktok"]);
  assert.equal(packet.proofGate.minimumProofUrlsNeeded, 2);
  assert.equal(packet.proofGate.fastPathAvailable, true);
  assert.equal(packet.proofGate.nextSafeButton, "preview_proof_links");
  assert.equal(packet.proofGate.nextLockedButton, "save_proof_links");
  assert.match(packet.proofGate.paths.oneScreenGuide, /proof-fill-one-screen\.txt$/);
  ...
  assert.match(packet.nextStep, /Fill SPORT and memes proof links/);
  assert.match(packet.nextStep, /Metricool\/Drive evidence/);
  assert.doesNotMatch(packet.nextStep, /public TikTok URLs and 24h metrics only after posts are live/);
  assert.equal(packet.nextRow, null);
  assert.equal(packet.blockedNextRow.metricoolQueueItemId, "53467d8f7dad");
  assert.equal(packet.blockedNextRow.blocker, "tiktok_metricool_proof_gate_not_ready");
  assert.doesNotMatch(JSON.stringify(packet.blockedNextRow), /Open Metricool|Upload\/review|schedule every row/i);
  ...
  assert.equal(packet.sourceStatuses.operatingRefresh, "blocked_external_account_proof");
  assert.equal(packet.sourceStatuses.metricool100Handoff, "ready_for_operator");
  assert.ok(packet.operatorSteps.some((step) => step.id === "proof-gate" && step.blocker === "tiktok_metricool_proof_gate_not_ready"));
  assert.ok(packet.operatorSteps.some((step) => step.id === "account-closeout" && step.blocker === "tiktok_account_closeout_not_ready"));
  assert.ok(packet.operatorSteps.some((step) => step.id === "metricool-batch" && step.action.includes("Do not open Metricool scheduling")));
  ...
  assert.ok(packet.guardrails.some((guardrail) => guardrail.includes("proof gate must be ready")));
  ...
  assert.match(markdown, /Proof Gate/);
  assert.match(markdown, /Status: blocked_needs_real_metricool_tiktok_proof/);
  ...
  assert.match(markdown, /Blocked queue item: 53467d8f7dad/);
  assert.doesNotMatch(markdown, /Open Metricool brand|Upload\/review the local TikTok source file/);
  ...
  assert.match(csv, /proof-gate/);
  assert.match(csv, /tiktok_metricool_proof_gate_not_ready/);
  ...
  assert.match(goLiveScript, /script\/clippers-tiktok-mvp-operating-refresh\.ts/);
  assert.match(goLiveScript, /proofGateReady/);
  assert.match(goLiveScript, /controlFieldsPresent/);
  ...
  assert.match(page, /clippers-tiktok-mvp-go-live-proof-gate/);
  assert.match(page, /Next row locked/);
...
test("TikTok MVP go-live packet stays in progress when Metricool 100 is blocked", async () => {
  ...
  const runbookPath = path.join(rootDir, "reports/clippers-tiktok-batch-runbook.json");
  const trackerPath = path.join(rootDir, "reports/clippers-tiktok-batch-tracker.json");
  const evidenceChecklistPath = path.join(rootDir, "reports/clippers-tiktok-evidence-checklist.json");
  const syncPath = path.join(rootDir, "reports/clippers-tiktok-batch-evidence-sync.json");
  ...
  const originalTracker = await readFile(trackerPath, "utf8");
  const originalEvidenceChecklist = await readFile(evidenceChecklistPath, "utf8");
  const originalSync = await readFile(syncPath, "utf8");
  try {
    ...
    const accountCloseout = JSON.parse(originalAccountCloseout);
    accountCloseout.status = "ready_for_metricool_tiktok";
    accountCloseout.totals = { ...(accountCloseout.totals || {}), rows: 2, ready: 2 };
    accountCloseout.rows = [
      { accountId: "sports-daily", accountName: "Sports Daily Clips", category: "sports", platform: "tiktok", handle: "@sportsdaily", status: "ready_for_metricool_tiktok", metricoolConnected: true, metricoolBrandOrProfile: "tiktok" },
      { accountId: "meme-radar", accountName: "Meme Radar", category: "memes", platform: "tiktok", handle: "@memeradar", status: "ready_for_metricool_tiktok", metricoolConnected: true, metricoolBrandOrProfile: "memes" },
    ];
    await writeFile(accountCloseoutPath, JSON.stringify(accountCloseout, null, 2));
    ...
    const tracker = JSON.parse(originalTracker);
    tracker.status = "ready_for_metricool_review";
    tracker.totals = { ...(tracker.totals || {}), readyToImport: 0 };
    await writeFile(trackerPath, JSON.stringify(tracker, null, 2));

    const evidenceChecklist = JSON.parse(originalEvidenceChecklist);
    evidenceChecklist.status = "ready_for_metricool_operator";
    await writeFile(evidenceChecklistPath, JSON.stringify(evidenceChecklist, null, 2));
    ...
    await writeFile(trackerPath, originalTracker);
    await writeFile(evidenceChecklistPath, originalEvidenceChecklist);
    await writeFile(syncPath, originalSync);
  }
});

test("TikTok MVP go-live packet fails closed on malformed ready proof gate", async () => {
  const operatingRefreshPath = path.join(rootDir, "reports/tiktok-mvp-operating-refresh/operating-refresh.json");
  const accountCloseoutPath = path.join(rootDir, "tiktok-mvp-account-closeout.json");
  const runbookPath = path.join(rootDir, "reports/clippers-tiktok-batch-runbook.json");
  const trackerPath = path.join(rootDir, "reports/clippers-tiktok-batch-tracker.json");
  ...
  try {
    ...
    const output = JSON.parse(result.stdout);
    assert.equal(output.status, "in_progress");
    assert.equal(output.proofGateStatus, "ready_for_operator_review");
    assert.equal(output.proofGateControlFieldsPresent, false);
    assert.equal(output.metricool100OperatorGateOpen, false);
    assert.equal(output.blockedNextRowBlocker, "tiktok_metricool_proof_gate_not_ready");
  } finally {
    await writeFile(operatingRefreshPath, originalOperatingRefresh);
    await writeFile(accountCloseoutPath, originalAccountCloseout);
    await writeFile(runbookPath, originalRunbook);
...
test("TikTok MVP go-live packet refreshes stale non-TikTok or inactive-account batch artifacts", async () => {
  ...
  try {
    ...
    assert.equal(inactivePacket.nextRow, null);
    assert.equal(inactivePacket.blockedNextRow.blocker, "tiktok_metricool_proof_gate_not_ready");
  } finally {
    await writeFile(runbookPath, originalRunbook);
    await writeFile(trackerPath, originalTracker);
    if (originalPacket) {
...
test("TikTok operator cockpit links upload and evidence consoles without publishing", async () => {
  ...
  assert.match(page, /clippers-tiktok-next-action-operating-refresh/);
  ...
  assert.match(page, /operatingRefresh/);
  ...
  assert.match(page, /!url\.username/);
  assert.match(page, /!url\.password/);
  assert.match(page, /profile_url and proof must be safe https URLs/);
  ...
  assert.match(page, /hostname === "drive\.google\.com"/);
  assert.match(page, /hostname === "docs\.google\.com"/);
  assert.match(page, /profile_url must be a public TikTok profile URL/);
  assert.match(page, /proof must be a Metricool HTTPS proof URL or Google Drive\/Docs evidence URL/);
...
test("TikTok next action surfaces Metricool proof bridge blocker when account lanes are blocked", async () => {
  ...
  try {
    ...
    const operatingRefreshRun = spawnSync(process.execPath, ["--import", "tsx", "script/clippers-tiktok-mvp-operating-refresh.ts"], {
      cwd: process.cwd(),
      encoding: "utf8",
    });
    assert.equal(operatingRefreshRun.status, 0, operatingRefreshRun.stderr || operatingRefreshRun.stdout);
    ...
    assert.equal(nextAction.operatingRefresh.status, "blocked_external_account_proof");
    assert.equal(nextAction.operatingRefresh.launchDecision, "blocked_before_metricool");
    ...
    assert.match(markdown, /Operating Refresh/);
    ...
    else await unlink(operatingRefreshPath).catch(() => undefined);
  }
});
...
test("goal completion audit keeps TikTok MVP honest while external work remains", async () => {
  ...
  assert.ok(["missing", "ready_for_save", "blocked_preview_not_clean"].includes(output.proofLinksPreviewGateStatus));
  assert.equal(typeof output.proofLinksPreviewGateFresh, "boolean");
  ...
  assert.ok(["missing", "ready_for_save", "blocked_preview_not_clean"].includes(audit.tiktokMvpProofLinksPreviewGate.status));
  assert.equal(typeof audit.tiktokMvpProofLinksPreviewGate.fresh, "boolean");
  assert.ok([false, "unknown"].includes(audit.tiktokMvpProofLinksPreviewGate.rawStored));
  assert.equal(
    audit.tiktokMvpProofLinksPreviewGate.rawStoredExplicitFalse,
    audit.tiktokMvpProofLinksPreviewGate.rawStored === false,
  );
  ...
  assert.match(currentBatchRequirement?.nextAction || "", /proof links|proof gate|Proof refresh/i);
  ...
  assert.ok(audit.operatorNextActions.every((row) => !/password=|access_token=|refresh_token=|client_secret=|cookie=|ready_to_send|realPublishEnabled=true|\bautopublish\b/i.test(JSON.stringify(row))));
  ...
  assert.match(markdown, /Fast path packet: .*proof-links-fast-path-paste-packet\.txt/);
  ...
  assert.match(markdown, /TikTok proof links preview gate status: (missing|ready_for_save|blocked_preview_not_clean)/);
  assert.match(markdown, /TikTok proof links preview gate fresh: (true|false)/);
  ...
  assert.doesNotMatch(markdown, /\bautopublish\b|ready_to_send/i);
  ...
  assert.match(nextActionsCsv, /fast_path_packet_path/);
  assert.match(nextActionsCsv, /proof-links-fast-path-paste-packet\.txt/);
  ...
  assert.doesNotMatch(nextActionsCsv, /password=|access_token=|refresh_token=|client_secret=|cookie=|ready_to_send|realPublishEnabled=true|\bautopublish\b/i);
  ...
  assert.match(page, /goalCompletionPrimaryAction\?\.fastPathPacketText/);
  assert.match(page, /Metricool proof lines copied/);
  assert.match(page, /load-clippers-goal-primary-proof-fast-path-button/);
  assert.match(page, /loadGoalMetricoolProofFastPath/);
  ...
  assert.match(page, /Fast-path packet:/);
  assert.match(page, /goalCompletionPrimaryFastPathLines/);
  assert.match(page, /fastPathPacketPath\?: string/);
  assert.match(page, /fastPathPacketText\?: string/);
...
test("goal completion audit fails closed on malformed proof links preview raw storage flags", async () => {
  const previewGatePath = path.join(rootDir, "reports/tiktok-mvp-proof-intake/proof-links-preview-gate.json");
  const originalGate = await readFile(previewGatePath, "utf8").catch(() => null);
  await mkdir(path.dirname(previewGatePath), { recursive: true });
  try {
    for (const rawStored of [undefined, "true"]) {
      const gate = {
        ...
        realPublishEnabled: false,
        rawHash: "safe-preview-hash",
        totals: {
          issues: 0,
          readyProofFields: 4,
          totalProofFields: 4,
        },
        nextStep: "Synthetic malformed gate for test.",
      } as Record<string, unknown>;
      if (rawStored !== undefined) gate.rawStored = rawStored;
      await writeFile(previewGatePath, JSON.stringify(gate, null, 2));
      ...
      assert.equal(result.status, 0, result.stderr || result.stdout);
      const audit = JSON.parse(await readFile(path.join(rootDir, "reports/clippers-goal-completion-audit.json"), "utf8"));
      assert.equal(audit.tiktokMvpProofLinksPreviewGate.status, "ready_for_save");
      assert.equal(audit.tiktokMvpProofLinksPreviewGate.readyForSave, false);
      assert.equal(audit.tiktokMvpProofLinksPreviewGate.rawStored, "unknown");
      assert.equal(audit.tiktokMvpProofLinksPreviewGate.rawStoredExplicitFalse, false);
      const proofGateRequirement = audit.requirements.find((row: { id: string }) => row.id === "tiktok_metricool_proof_gate");
      assert.match(proofGateRequirement?.evidence || "", /proofLinksPreviewGateReadyForSave false/);
      assert.match(proofGateRequirement?.evidence || "", /proofLinksPreviewRawStored unknown/);
    }
  } finally {
    if (originalGate === null) {
      await rm(previewGatePath, { force: true });
    } else {
      await writeFile(previewGatePath, originalGate);
    }
...
test("goal completion audit rejects stale ready proof gates with hidden blockers", async () => {
  ...
  try {
    ...
    assert.equal(audit.operatorNextActions[0].title, "Fast path Metricool TikTok proof gate");
    assert.doesNotMatch(audit.operatorNextActions[0].buttonOrFile, /proof-refresh\.md$/);
    ...
    assert.doesNotMatch(malformedAudit.operatorNextActions[0].buttonOrFile, /proof-refresh\.md$/);
    ...
    await writeFile(proofRefreshPath, JSON.stringify({
      ...
      blockers: ["doctor_status_needs_proof_fix", "lanes_ready_1_of_2", "doctor_fix_queue_1"],
      paths: {
    ...
    const blockedRefreshResult = spawnSync(process.execPath, ["script/clippers-goal-completion-audit.mjs"], {
      cwd: process.cwd(),
      encoding: "utf8",
    });
    assert.equal(blockedRefreshResult.status, 0, blockedRefreshResult.stderr || blockedRefreshResult.stdout);
    const blockedRefreshAudit = JSON.parse(await readFile(path.join(rootDir, "reports/clippers-goal-completion-audit.json"), "utf8"));
    assert.equal(blockedRefreshAudit.status, "not_ready");
    assert.equal(blockedRefreshAudit.fullGoal.tiktokMvpOperatingProofGateReady, true);
    assert.equal(blockedRefreshAudit.fullGoal.tiktokMvpProofRefreshReady, false);
    assert.equal(blockedRefreshAudit.fullGoal.tiktokMvpProofGateReady, false);
    assert.equal(blockedRefreshAudit.fullGoal.metricoolMvpReady, false);
    assert.equal(blockedRefreshAudit.tiktokMvpProofGate.status, "ready_for_operator_review");
    assert.equal(blockedRefreshAudit.tiktokMvpProofRefresh.status, "blocked");
    assert.ok(blockedRefreshAudit.tiktokMvpProofRefresh.blockers.includes("doctor_status_needs_proof_fix"));
    assert.equal(blockedRefreshAudit.requirements.find((row) => row.id === "tiktok_metricool_proof_gate")?.status, "needs_external_action");
    assert.match(blockedRefreshAudit.requirements.find((row) => row.id === "tiktok_metricool_proof_gate")?.evidence || "", /proofGateReady true/);
    assert.match(blockedRefreshAudit.requirements.find((row) => row.id === "tiktok_metricool_proof_gate")?.evidence || "", /proofRefreshReady false/);
    assert.equal(blockedRefreshAudit.requirements.find((row) => row.id === "tiktok_batch_01_ready_for_metricool")?.status, "needs_external_action");
    assert.equal(blockedRefreshAudit.operatorNextActions[0].title, "Fast path Metricool TikTok proof gate");
    assert.match(blockedRefreshAudit.operatorNextActions[0].buttonOrFile, /proof-refresh\.md$/);
    assert.match(blockedRefreshAudit.operatorNextActions[0].nextAction, /doctor_status_needs_proof_fix/);
    assert.match(blockedRefreshAudit.nextStep, /proof refresh doctor queue/);

    await unlink(proofRefreshPath);
    const missingRefreshResult = spawnSync(process.execPath, ["script/clippers-goal-completion-audit.mjs"], {
      cwd: process.cwd(),
      encoding: "utf8",
    });
    assert.equal(missingRefreshResult.status, 0, missingRefreshResult.stderr || missingRefreshResult.stdout);
    const missingRefreshAudit = JSON.parse(await readFile(path.join(rootDir, "reports/clippers-goal-completion-audit.json"), "utf8"));
    assert.equal(missingRefreshAudit.status, "not_ready");
    ...
    assert.equal(staleRefreshResult.status, 0, staleRefreshResult.stderr || staleRefreshResult.stdout);
    const staleRefreshAudit = JSON.parse(await readFile(path.join(rootDir, "reports/clippers-goal-completion-audit.json"), "utf8"));
    assert.equal(staleRefreshAudit.status, "not_ready");
    assert.equal(staleRefreshAudit.fullGoal.tiktokMvpOperatingProofGateReady, true);
    assert.equal(staleRefreshAudit.fullGoal.tiktokMvpProofRefreshReady, false);
    assert.equal(staleRefreshAudit.fullGoal.tiktokMvpProofGateReady, false);
    assert.equal(staleRefreshAudit.tiktokMvpProofRefresh.status, "ready_to_apply");
    assert.equal(staleRefreshAudit.tiktokMvpProofRefresh.fresh, false);
    assert.ok(staleRefreshAudit.tiktokMvpProofRefresh.blockers.includes("proof_refresh_stale_or_missing"));
    assert.equal(staleRefreshAudit.requirements.find((row) => row.id === "tiktok_metricool_proof_gate")?.status, "needs_external_action");
