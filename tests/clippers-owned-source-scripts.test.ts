...
test("TikTok MVP go-live packet combines account closeout and batch runbook safely", async () => {
  ...
  assert.equal(output.metricool100ReadyBatches, 10);
  assert.equal(output.metricool100BlockedBatches, 0);
  assert.equal(output.conflicts, 0);
  ...
  assert.equal(packet.totals.metricool100ReadyBatches, 10);
  assert.equal(packet.totals.metricool100BlockedBatches, 0);
  ...
  assert.equal(packet.metricool100.readyBatches, 10);
  assert.equal(packet.metricool100.blockedBatches, 0);
  ...
  assert.match(markdown, /Ready batches: 10/);
...
test("TikTok MVP readiness verifier gates Metricool operator start without fake publishing", async () => {
  ...
  assert.match(pageSource, /Importable evidence/);
  assert.doesNotMatch(pageSource, />Published<\/p>[\s\S]{0,160}readyToImport/);
  assert.match(pageSource, /Local MP4 staging only\. This does not mean scheduled, published, rights-cleared, or proven\./);
  assert.match(pageSource, /local MP4 verified/);
