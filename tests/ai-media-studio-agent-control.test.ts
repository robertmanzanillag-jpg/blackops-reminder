...
test("dedicated media agent snapshot exposes exact ownership, gates, evidence and launch bounds", () => {
  ...
  assert.equal(snapshot.agent.status, "awaiting_human");
  assert.equal(snapshot.dataAsOf, "2026-07-22T20:11:43.000Z");
  ...
  assert.equal(snapshot.summary.review, 30);
  assert.equal(snapshot.summary.merged, 0);
...
test("agent control exposes the exact open draft chain through PR 185 without merged claims", () => {
  const snapshot = createAiMediaStudioAgentSnapshot(() => new Date("2026-07-22T21:00:00.000Z"));
  const expected = [
    [174, "codex/ai-media-studio-source-to-batch-automation", "codex/ai-media-studio-kong-http-scheduler"],
    [175, "codex/ai-media-studio-kong-http-scheduler", "codex/ai-media-studio-heygen-setup-ux"],
    [178, "codex/ai-media-studio-heygen-setup-ux", "codex/ai-media-studio-production-batch-route-hardening"],
    [180, "codex/ai-media-studio-production-batch-route-hardening", "codex/ai-media-studio-agent-control-checkpoint"],
    [181, "codex/ai-media-studio-agent-control-checkpoint", "codex/ai-media-studio-content-plan-policy"],
    [182, "codex/ai-media-studio-content-plan-policy", "codex/ai-media-studio-launch-plan-profile"],
    [183, "codex/ai-media-studio-launch-plan-profile", "codex/ai-media-studio-dashboard-published-metric"],
    [184, "codex/ai-media-studio-dashboard-published-metric", "codex/ai-media-studio-reusable-script-assets"],
    [185, "codex/ai-media-studio-reusable-script-assets", "codex/ai-media-studio-provider-neutral-persona-activation"],
  ] as const;

  const chain = expected.map(([number, baseBranch, headBranch]) => {
    const item = snapshot.workItems.find((candidate) => candidate.pullRequestUrl?.endsWith(`/pull/${number}`));
    assert.ok(item, `missing PR #${number}`);
    assert.equal(item.state, "review");
    assert.equal(item.baseBranch, baseBranch);
    assert.equal(item.headBranch, headBranch);
    assert.notEqual(item.gates?.human.status, "passed");
    return item;
  });
  assert.equal(snapshot.summary.merged, 0);
  assert.equal(chain.some((item) => item.state === "merged"), false);
  assert.doesNotMatch(JSON.stringify(snapshot), /"state":"done"/u);
  const current = chain.at(-1);
  assert.equal(current?.gates?.checker.status, "passed");
  assert.equal(current?.gates?.appQa.status, "passed");
  assert.equal(current?.gates?.ci.status, "pending");
  assert.equal(current?.gates?.human.status, "pending");
});
...
test("agent control records the real draft checkpoint chain and separately approved launch gates", () => {
  ...
  assert.equal(setup?.state, "review");
  ...
  assert.equal(security?.state, "review");
...
test("agent control surfaces durable scheduler progress and dead-letter state without private payloads", () => {
  ...
  assert.equal(item?.state, "review", "runtime health must not rewrite the open PR delivery state");
  assert.deepEqual(item?.runtime, {
    component: "source_scheduler",
    status: "dead_letter",
    health: "attention",
    evidence: [
      "Runtime scheduler state: dead_letter",
      "Durable cursor page 2; cycle 3; attempts 5/5",
      "Safe failure code: source_sync_unavailable",
    ],
  });
...
test("agent control reports an uninitialized scheduler as stopped runtime without changing review state", () => {
  const snapshot = createAiMediaStudioAgentSnapshot(() => new Date("2026-07-23T20:00:00.000Z"));
  const item = snapshot.workItems.find((work) => work.id === "ams-agent-kong-source-scheduler");
  assert.equal(item?.state, "review");
  assert.equal(item?.runtime?.status, "not_initialized");
  assert.equal(item?.runtime?.health, "stopped");
  assert.equal(snapshot.summary.blocked, 2);
});
...
test("agent status is explicitly no-spend, no-deploy, no-migration and no-live-provider", () => {
  ...
  assert.equal(durablePlan?.state, "review");
  ...
  assert.equal(durableScriptBatch?.state, "review");
  ...
  assert.equal(launchReadiness?.state, "review");
  ...
  assert.equal(offlinePreflight?.state, "review");
  ...
  assert.equal(sandboxReadiness?.state, "review");
  ...
  assert.equal(staticHeyGenOnboarding?.state, "review");
  ...
  assert.equal(oneVideoExecutionControl?.state, "review");
  ...
  assert.equal(heyGenVerificationEvidence?.state, "review");
  ...
  assert.equal(secureHeyGenSetupRuntime?.state, "review");
  ...
  assert.equal(quoteBoundHumanApproval?.state, "review");
  ...
  assert.equal(heyGenAccountMaximumQuote?.state, "review");
  ...
  assert.equal(heldAdmission?.state, "review");
  ...
  assert.equal(rosterMutationHardening?.state, "review");
  ...
  assert.equal(sourceAutomationSync?.state, "review");
  ...
  assert.equal(kongSourceToScript?.state, "review");
...
test("snapshot schema rejects summary drift and unsafe PR hosts", () => {
  ...
  assert.equal(aiMediaStudioAgentSnapshotSchema.safeParse({
    ...snapshot,
    workItems: snapshot.workItems.map((item, index) => index === 0
      ? { ...item, state: "merged", gates: { ...item.gates!, human: { status: "pending", evidence: [] } } }
      : item),
  }).success, false);
  assert.equal(aiMediaStudioAgentSnapshotSchema.safeParse({
    ...snapshot,
    workItems: snapshot.workItems.map((item, index) => index === 0
      ? { ...item, gates: { ...item.gates!, human: { status: "passed", evidence: ["Unverified claim"] } } }
      : item),
  }).success, false);
});
