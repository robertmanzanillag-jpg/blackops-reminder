...
test("Revenue Engine exposes GitHub handoff route for sold website workspaces", () => {
  ...
  assert.match(routeSource, /workspace\.input\.publicDataVerified/);
  assert.match(routeSource, /workspace\.projectPlan\.decision\.status !== "ready_to_build"/);
  assert.match(routeSource, /Verifica data publica/);
...
test("Revenue Engine exposes the daily money command panel", () => {
  ...
  assert.match(uiSource, /const activeScoutTask = snapshot\?\.latestDailyScoutSprint\?\.tasks\.find/);
