...
import { mediaJobResponseSchema, mediaJobsResponseSchema, mediaStudioOptionsResponseSchema } from "../shared/ai-media-studio";
...
test("AI Media Studio HTTP routes enforce auth, schemas and signed webhook transitions", async (t) => {
  ...
  assert.equal((await invalid.json() as { code: string }).code, "PLAN_ADMISSION_REQUIRED");
  ...
  const blockedGeneration = await fetch(`${baseUrl}/api/ai-media-studio/generations`, { method: "POST", headers: jsonHeaders, body: JSON.stringify(generationBody) });
  assert.equal(blockedGeneration.status, 409);
  const created = await repository.update({
    ...await runtime.service.createGeneration("user-a", generationBody),
    influencerName: "Emily",
  });
  assert.equal(created.influencerName, "Emily");
  assert.equal(created.status, "rendering");
  ...
  const webhookBody = JSON.stringify({ event_type: "avatar_video.success", event_data: { video_id: created.id, video_url: "https://cdn.example.com/render.mp4" } });
  ...
  assert.equal((await runtime.service.getJob("user-a", created.id)).status, "rendering");

  const internalJob = await runtime.service.getJob("user-a", created.id);
  ...
  const detail = mediaJobResponseSchema.parse(await (await fetch(`${baseUrl}/api/ai-media-studio/jobs/${created.id}`, { headers: { "x-test-user": "user-a" } })).json());
  ...
  assert.equal((await fetch(`${baseUrl}/api/ai-media-studio/jobs/${created.id}/cancel`, { method: "POST", headers: { "x-test-user": "user-a" } })).status, 409);
  ...
  const retryTarget = await runtime.service.createGeneration("user-a", { ...generationBody, idempotencyKey: "route-generation-002" });
  const retryInternal = await runtime.service.getJob("user-a", retryTarget.id);
  ...
  const blockedRetry = await fetch(`${baseUrl}/api/ai-media-studio/jobs/${retryTarget.id}/retry`, { method: "POST", headers: { "x-test-user": "user-a" } });
  assert.equal(blockedRetry.status, 409);
  assert.equal((await blockedRetry.json() as { code: string }).code, "PLAN_ADMISSION_REQUIRED");
  const retried = await runtime.service.retryJob("user-a", retryTarget.id);
  assert.equal(retried.status, "rendering");
  assert.equal(retried.attempts, 2);
  const cancelled = mediaJobResponseSchema.parse(await (await fetch(`${baseUrl}/api/ai-media-studio/jobs/${retryTarget.id}/cancel`, { method: "POST", headers: { "x-test-user": "user-a" } })).json());
  ...
  assert.equal(deniedGeneration.status, 409);
  assert.equal((await deniedGeneration.json() as { code: string }).code, "PLAN_ADMISSION_REQUIRED");
