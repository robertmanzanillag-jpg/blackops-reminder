...
test("reports dry-run research mode until production money blockers are configured", () => {
  ...
  assert.ok(report.allowedToday.some((item) => item.includes("guarded public subagent/browser scout capture")));
  assert.ok(report.allowedToday.some((item) => item.includes("no contact")));
  assert.ok(report.allowedToday.some((item) => item.includes("no paid data")));
  assert.ok(report.allowedToday.some((item) => item.includes("no scraping at scale")));
  assert.ok(report.allowedToday.some((item) => item.includes("no lead import")));
