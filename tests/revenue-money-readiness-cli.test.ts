...
const originalEnv = {
  ...
  REVENUE_ENGINE_WEBSITE_PR_EVIDENCE: process.env.REVENUE_ENGINE_WEBSITE_PR_EVIDENCE,
  REVENUE_ENGINE_WEBSITE_APP_QA_TARGET_PASSED: process.env.REVENUE_ENGINE_WEBSITE_APP_QA_TARGET_PASSED,
  REVENUE_ENGINE_WEBSITE_APP_QA_EVIDENCE: process.env.REVENUE_ENGINE_WEBSITE_APP_QA_EVIDENCE,
  REVENUE_ENGINE_WEBSITE_PREVIEW_DEPLOY_VERIFIED: process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_DEPLOY_VERIFIED,
  REVENUE_ENGINE_WEBSITE_PREVIEW_EVIDENCE: process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_EVIDENCE,
  REVENUE_ENGINE_WEBSITE_ROLLBACK_VERIFIED: process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_VERIFIED,
  REVENUE_ENGINE_WEBSITE_ROLLBACK_EVIDENCE: process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_EVIDENCE,
  REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVED_BY_ROBERT: process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVED_BY_ROBERT,
  REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVAL_EVIDENCE: process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVAL_EVIDENCE,
  REVENUE_ENGINE_DEPLOY_APPROVED_BY_ROBERT: process.env.REVENUE_ENGINE_DEPLOY_APPROVED_BY_ROBERT,
  REVENUE_ENGINE_DEPLOY_APPROVAL_EVIDENCE: process.env.REVENUE_ENGINE_DEPLOY_APPROVAL_EVIDENCE,
};
...
function clearWebsitePublishEnv() {
  delete process.env.REVENUE_ENGINE_WEBSITE_DEPLOY_ENABLED;
  delete process.env.REVENUE_ENGINE_WEBSITE_PR_EVIDENCE;
  delete process.env.REVENUE_ENGINE_WEBSITE_APP_QA_TARGET_PASSED;
  delete process.env.REVENUE_ENGINE_WEBSITE_APP_QA_EVIDENCE;
  delete process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_DEPLOY_VERIFIED;
  delete process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_EVIDENCE;
  delete process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_VERIFIED;
  delete process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_EVIDENCE;
  delete process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVED_BY_ROBERT;
  delete process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVAL_EVIDENCE;
  delete process.env.REVENUE_ENGINE_DEPLOY_APPROVED_BY_ROBERT;
  delete process.env.REVENUE_ENGINE_DEPLOY_APPROVAL_EVIDENCE;
}

function setReadyWebsitePublishEnv() {
  const appQaPath = path.join(process.cwd(), "revenue_workspace", "app-qa", "test-client-site-report.md");
  const rollbackPath = path.join(process.cwd(), "revenue_workspace", "rollback", "test-client-site.md");
  mkdirSync(path.dirname(appQaPath), { recursive: true });
  mkdirSync(path.dirname(rollbackPath), { recursive: true });
  writeFileSync(appQaPath, "# Test App QA evidence\n\nRoute, link, API, error and improvement scouts passed for test fixture.\n");
  writeFileSync(rollbackPath, "# Test rollback evidence\n\nRollback target and revert plan verified for test fixture.\n");
  process.env.REVENUE_ENGINE_WEBSITE_DEPLOY_ENABLED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_PR_EVIDENCE = "github-pr:robert/client-sites#17";
  process.env.REVENUE_ENGINE_WEBSITE_APP_QA_TARGET_PASSED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_APP_QA_EVIDENCE = "revenue_workspace/app-qa/test-client-site-report.md";
  process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_DEPLOY_VERIFIED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_EVIDENCE = "https://client-site-preview.replit.app";
  process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_VERIFIED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_EVIDENCE = "revenue_workspace/rollback/test-client-site.md";
  process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVED_BY_ROBERT = "true";
  process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVAL_EVIDENCE = "codex-thread:robert-publish-approval-17";
  process.env.REVENUE_ENGINE_DEPLOY_APPROVED_BY_ROBERT = "true";
  process.env.REVENUE_ENGINE_DEPLOY_APPROVAL_EVIDENCE = "codex-thread:robert-deploy-approval-17";
}

test.afterEach(restoreEnv);

test.afterEach(() => {
  rmSync(path.join(process.cwd(), "revenue_workspace", "app-qa", "test-client-site-report.md"), { force: true });
  rmSync(path.join(process.cwd(), "revenue_workspace", "rollback", "test-client-site.md"), { force: true });
});
...
test("first sprint readiness can be ready while production-only gaps remain", () => {
  ...
  assert.ok(report.remainingGaps.some((item) => item.includes("website PR evidence")));
});
...
test("does not treat website deploy enablement alone as publish-ready", () => {
  ...
  assert.equal(report.canBuildWebsites, false);
  assert.equal(report.checks.find((check) => check.id === "website_build_pipeline")?.status, "fail");
  assert.ok(report.blockedUntil.some((item) => item.includes("evidence")));
});

test("does not treat website publish booleans as ready without durable evidence refs", () => {
  process.env.DATABASE_URL = "postgres://ceo_user:real-pass@db.internal:5432/blackops";
  process.env.SESSION_SECRET = "a-production-session-secret-32-chars";
  process.env.REVENUE_ENGINE_MONEY_MODE = "live";
  process.env.REVENUE_ENGINE_ROBERT_CONTACT_APPROVED = "true";
...
test("requires website evidence refs to match the expected evidence type", () => {
  process.env.DATABASE_URL = "postgres://ceo_user:real-pass@db.internal:5432/blackops";
  process.env.SESSION_SECRET = "a-production-session-secret-32-chars";
  process.env.REVENUE_ENGINE_MONEY_MODE = "live";
  process.env.REVENUE_ENGINE_ROBERT_CONTACT_APPROVED = "true";
  ...
  process.env.REVENUE_ENGINE_PAYMENT_SMOKE_VERIFIED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_DEPLOY_ENABLED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_PR_EVIDENCE = "https://not-a-pr.invalid/review";
  process.env.REVENUE_ENGINE_WEBSITE_APP_QA_TARGET_PASSED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_APP_QA_EVIDENCE = "stripe:deposit-123";
  process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_DEPLOY_VERIFIED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_PREVIEW_EVIDENCE = "https://preview.example.com";
  process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_VERIFIED = "true";
  process.env.REVENUE_ENGINE_WEBSITE_ROLLBACK_EVIDENCE = "revenue_workspace/rollback/missing-client-site.md";
  process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVED_BY_ROBERT = "true";
  process.env.REVENUE_ENGINE_WEBSITE_PUBLISH_APPROVAL_EVIDENCE = "https://random.invalid/approval";
  process.env.REVENUE_ENGINE_DEPLOY_APPROVED_BY_ROBERT = "true";
  process.env.REVENUE_ENGINE_DEPLOY_APPROVAL_EVIDENCE = "https://random.invalid/deploy";
