...
test("ensureAccountShape preserves verified state only for the expected Metricool TikTok profile", () => {
  const expected = __clipperInternals.ensureAccountShape({
    id: "sports-daily",
    name: "Old account label",
    category: "sports",
    platforms: ["TikTok", "Instagram"],
    platformAccounts: [{
      platform: "tiktok",
      handle: "@streamersclipusa",
      displayName: "Public profile",
      status: "ready",
      requiredScopes: [],
      missingSteps: [],
      notes: "Verified through the connected Metricool profile.",
    }],
    dailyClipTarget: 17,
    weeklyViewsGoal: 2_000_000,
    lastWeekViews: 321_000,
    lastWeekViewsVerified: true,
    status: "ready",
    contentPolicy: "legacy",
  });

  assert.equal(expected.name, "Streamer Highlights");
  assert.deepEqual(expected.platforms, ["TikTok"]);
  assert.equal(expected.dailyClipTarget, 17);
  assert.equal(expected.weeklyViewsGoal, 2_000_000);
  assert.equal(expected.platformAccounts[0].handle, "@streamersclipusa");
  assert.equal(expected.platformAccounts[0].status, "ready");
  assert.equal(expected.lastWeekViews, 321_000);
  assert.equal(expected.lastWeekViewsVerified, true);
  assert.equal(expected.status, "ready");

  const wrongProfile = __clipperInternals.ensureAccountShape({
    ...expected,
    platformAccounts: [{ ...expected.platformAccounts[0], handle: "@unverified-profile", status: "ready" }],
    lastWeekViews: 999_999,
    lastWeekViewsVerified: false,
    status: "ready",
  });
  assert.equal(wrongProfile.platformAccounts[0].handle, "@streamersclipusa");
  assert.equal(wrongProfile.platformAccounts[0].status, "needs_review");
  assert.equal(wrongProfile.lastWeekViews, 0);
  assert.equal(wrongProfile.lastWeekViewsVerified, false);
  assert.equal(wrongProfile.status, "needs_connection");
});

test("Metricool launch handles must match the two expected TikTok profiles", () => {
  assert.equal(__clipperInternals.metricoolTikTokHandleMatchesAccount("sports-daily", "@streamersclipusa"), true);
  assert.equal(__clipperInternals.metricoolTikTokHandleMatchesAccount("meme-radar", "streamersclips"), true);
  assert.equal(__clipperInternals.metricoolTikTokHandleMatchesAccount("sports-daily", "@wrong-sports-profile"), false);
  assert.equal(__clipperInternals.metricoolTikTokHandleMatchesAccount("meme-radar", ""), false);
});
...
test("getClipperStatus exposes guarded growth goals", async () => {
  ...
  assert.equal(status.accountLaunchKit.tasks.length, status.accounts.reduce((sum, account) => sum + account.platformAccounts.length, 0));
  assert.ok(status.growthAudit.items.some((item) => item.id === "account-launch-kit"));
  assert.equal(status.accountEvidence.totals.expected, status.accounts.reduce((sum, account) => sum + account.platformAccounts.length, 0));
...
test("prepareClipperExternalAccountPermissionSprint aliases the 100 clips sprint and sanitizes evidence rows", async () => {
  ...
  try {
    ...
    assert.ok(sprint.items.some((item) => item.type === "account_create" || item.type === "account_verify"));
    assert.ok(sprint.items.some((item) => item.type === "permission_request"));
    const metricoolItems = sprint.items.filter((item) => item.type === "metricool_profile" || item.type === "metricool_queue");
...
test("bootstrapClipperAccounts prepares platform accounts and permission queue", async () => {
  ...
  assert.equal(platformAccounts.length, status.accounts.reduce((sum, account) => sum + account.platformAccounts.length, 0));
  assert.ok(platformAccounts.every((account) => account.platform === "tiktok"));
  assert.ok(platformAccounts.every((account) => account.requiredScopes.length === 0));
  assert.equal(status.permissionQueue.some((permission) => permission.scope === "instagram_content_publish"), false);
...
test("prepareClipperAccountIdentityKit writes profiles, prompts and first post ideas", async () => {
  ...
  try {
    ...
    assert.equal(accountIdentityKit.totals.platformProfiles, status.accounts.reduce((sum, account) => sum + account.platformAccounts.length, 0));
...
test("prepareClipperAccountLaunchKit writes account creation manifest", async () => {
  ...
  try {
    assert.equal(accountLaunchKit.tasks.length, status.accounts.reduce((sum, account) => sum + account.platformAccounts.length, 0));
...
test("prepareClipperAccountCreationPack writes executable account creation CSV and runbook", async () => {
  ...
  try {
    ...
    assert.ok(accountCreationPack.items.every((item) => item.platform === "tiktok"));
    ...
    assert.equal(accountCreationPack.platformBatches.length, 1);
    assert.equal(accountCreationPack.totals.platformBatches, accountCreationPack.platformBatches.length);
    assert.ok(accountCreationPack.platformBatches.some((batch) => batch.platform === "tiktok" && batch.handles.includes("@streamersclipusa") && batch.handles.includes("@streamersclips")));
...
test("prepareClipperMetricoolPublishingPlan writes Sports and Memes Metricool launch channels", async () => {
  ...
  const fetchMock = mock.method(globalThis, "fetch", async () => new Response(JSON.stringify({ data: connectedMetricoolBrands }), { status: 200, headers: { "content-type": "application/json" } }));
  ...
  try {
    ...
    assert.ok(metricoolPublishing.channels.every((channel) => channel.networks.length === 1 && channel.networks[0] === "tiktok"));
    ...
    assert.ok(metricoolPublishing.channels.every((channel) => channel.permissionsToGrant.length === 0));
    ...
    connectedMetricoolBrands = connectedMetricoolBrands.map((brand) => brand.id === 6431687
      ? { ...brand, networksData: { tiktokData: "wrong-sports-profile" } }
      : brand);
    const wrongProfile = await prepareClipperMetricoolPublishingPlan();
    const blockedSports = wrongProfile.metricoolPublishing.channels.find((channel) => channel.accountId === "sports-daily");
    assert.equal(blockedSports?.publishGate, "blocked");
    assert.ok(blockedSports?.blockers.some((blocker) => blocker.includes("@streamersclipusa") && blocker.includes("@wrong-sports-profile")));
    assert.equal(wrongProfile.metricoolPublishing.status, "ready_to_connect");
