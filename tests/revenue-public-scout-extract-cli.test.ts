...
test("rejects sensitive output paths and existing outputs without overwrite", () => {
  ...
  const fakeChecks = {
    exists: (path: string) => path === "/tmp" || path === "/tmp/extracted.json",
    lstat: () => ({ isFile: () => true, isSymbolicLink: () => false }),
    realpath: (path: string) => path,
  };
  assert.deepEqual(validateRevenuePublicScoutExtractOptions(existingOutput, fakeChecks), [
    "--output already exists; pass --overwrite to replace it.",
  ]);
  assert.deepEqual(validateRevenuePublicScoutExtractOptions(overwriteOutput, fakeChecks), []);
  assert.deepEqual(validateRevenuePublicScoutExtractOptions(parseRevenuePublicScoutExtractArgs([
    "--input=/tmp/public-notes.txt",
    "--output=/Users/robertmanzanilla/Desktop/extracted.json",
  ]), {
    exists: (path: string) => path === "/Users/robertmanzanilla/Desktop",
...
test("formats public scout extract text with safe next command", () => {
  ...
  assert.match(output, /npm \["run","revenue:public-scout-run"/);
  assert.match(output, /"--input=\/tmp\/extracted\.json"/);
  assert.doesNotMatch(output, /npm run revenue:public-scout-run -- --input=/);
