...
| PR26 DB capability + race proof | PR #109, `codex/ai-media-studio-db-capability-races`, stacked on PR #108 | App QA / GitHub checkpoint | Function-only least-privilege PostgreSQL API, exact owner+workspace and `SESSION_USER` capabilities, split submit/reconcile lanes, hostile-result binding, durable active capacity and exact refund/terminal release pass 15 adapter checks, 8 migration checks, 615 full-suite checks plus 20 PostgreSQL-only skips, inherited PostgreSQL 16 at 13/13 and PR26 at 7/7 including live fail-closed rollback, TypeScript/build/map/diff hygiene and independent SQL review with no P0/P1. App QA remains active. SQL is unapplied; no route/runtime/provider/spend/deploy is enabled, and HeyGen terminal evidence plus approved sandbox remain gates |
