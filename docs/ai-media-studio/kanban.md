...
| PR16 provider activation CAS | PR #97, `codex/ai-media-studio-provider-activation-cas`, stacked on PR #94 | Draft / GitHub checkpoint | Exact selected-target/account activation domain CAS, one-secret-per-role vault v2 and additive artifact/binding/vault-operation schema are preserved remotely. Focused and App QA regressions pass, but reviewed migration SQL, the Drizzle activation transaction and cleanup-vs-activation fencing remain blockers; legacy publishing stays fail-closed, and routes, workers, real AWS/provider calls and deployment remain absent |
...
| AMS-311 OAuth/vault control plane | Identity owner | PR16 in progress | Durable state/PKCE policy, exact account binding, fenced callback saga, encrypted code/token adapters, cleanup obligations, infrastructure preflight and an explicit provider-target selection handoff are preserved in the stacked series. Local PR16 work adds activation domain CAS, role/target-bound vault v2 and additive schema declarations; migration SQL, Drizzle activation, runtime wiring, cleanup v2 composition, effective IAM proof, real connectors, refresh/revocation, routes and sandbox proof remain blocked |
