...
| PR14 OAuth vault operations | `codex/ai-media-studio-oauth-vault-operations`, stacked on PR #89 | Local gate / GitHub checkpoint pending | Durable exact cleanup obligations, fenced two-pass reconciler and inert double-snapshot S3/KMS preflight are implemented. Migration is unapplied; no route, timer, runtime wiring, live AWS/provider call or deployment is enabled |
...
| AMS-311 OAuth/vault control plane | Identity owner | PR14 local gate | Durable state/PKCE policy, exact account binding, fenced callback saga, encrypted code/token adapters, cleanup obligations and infrastructure preflight are implemented in the stacked series. Runtime wiring, effective IAM proof, target-selection contract, real connectors, refresh/revocation, routes and sandbox proof remain blocked |
