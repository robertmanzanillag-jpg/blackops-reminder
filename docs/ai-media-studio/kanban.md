...
| PR20 durable launch authorities | `codex/ai-media-studio-durable-authorities`, stacked on PR #102 | Draft checkpoint ready | Append-only policy, kill-switch, evidence and exact snapshot records remove caller self-certification from the reservation request. A global advisory lock makes count-based concurrency admission race-safe, while dedicated counters remain a scale optimization. Full suite 543/543, TypeScript, build, independent checker and static App QA pass. Migration and repository remain unapplied/unmounted; authenticated issuers and live PostgreSQL migration/contention evidence remain blockers. No job, outbox, provider call, spend or deployment |
