...
| PR15 provider connection stages | PR #94, `codex/ai-media-studio-provider-connection-stages`, stacked on PR #92 | Draft / GitHub checkpoint | Provider-neutral exchange/discovery/explicit-selection/activation-handoff stages, safe token artifact descriptors and additive target evidence tables passed 31/31 focused tests plus diff hygiene. Typecheck and full checker/security/App QA remain pending; no route, runtime wiring, provider call, migration apply or deployment |
| PR15 provider connection stages | `codex/ai-media-studio-provider-connection-stages`, stacked on PR #92 | Local review gates | Provider-neutral exchange/discovery/explicit-selection contracts and additive evidence tables are implemented without token/vault references. Migration is unapplied; activation, routes, live providers and deployment remain absent |
...
| AMS-311 OAuth/vault control plane | Identity owner | PR15 local gates | Durable state/PKCE policy, exact account binding, fenced callback saga, encrypted code/token adapters, cleanup obligations, infrastructure preflight and an explicit provider-target selection handoff are preserved in the stacked series. Runtime wiring, effective IAM proof, role-specific vault cleanup v2, activation/account CAS integration, real connectors, refresh/revocation, routes and sandbox proof remain blocked |
