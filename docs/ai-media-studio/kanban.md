...
| PR13 encrypted OAuth vaults | `codex/ai-media-studio-encrypted-oauth-vaults`, stacked on PR #88 | Checker/security/App QA passed | Application KMS envelope encryption plus S3 SSE-KMS for authorization codes and tokens, exact contexts, conditional recovery and minimum-privilege token reader pass local gates. Runtime wiring, hard IAM separation, lifecycle/reconciler and sandbox connectors remain blocked |
...
| AMS-311 OAuth/vault control plane | Identity owner | PR13 local gates passed | Durable state/PKCE policy, exact account binding, fenced callback saga and application-envelope-encrypted code/token adapters are implemented in the stacked branches. Runtime wiring, hard IAM role separation, lifecycle/reconciler, target-selection contract, real connectors, refresh/revocation, routes and sandbox proof remain blocked |
