...
| PR16 provider activation CAS | `codex/ai-media-studio-provider-activation-cas`, stacked locally on PR #94 | In progress / local only | Exact selected-target/account activation domain CAS, one-secret-per-role vault v2 and additive artifact/binding/vault-operation schema are implemented locally. Reviewed migration SQL and the Drizzle activation transaction remain pending; no remote PR number or test-gate result is claimed, legacy publishing stays fail-closed, and routes, workers, real AWS/provider calls and deployment remain absent |
