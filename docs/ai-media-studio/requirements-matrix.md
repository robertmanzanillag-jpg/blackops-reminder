...
| Database migration applied | Missing | PR30 plus local PR16A inventory all 20 available forward/rollback pairs in a no-go, statically checked staging runbook; no staging database connection, migration or `db:push` has run. Provenance audit proves PR13 is schema-neutral and PR14 depends on PR12 DB controls. PR16A has clean independent checker/App QA and isolated PostgreSQL 16 integrity proof but is unapplied; PR16B durable repository/CAS is absent | Resolve PR1 provenance, implement/review PR16B, merge the exact stack, identify staging, prove backup restore, provision separate least-privilege principals, obtain explicit approval, then run ordered forward/restart/reverse rehearsal with App QA as a hard deploy gate. Prove PR13 external storage readiness separately before restart/runtime/sandbox |
