...
| Durable daily budget admission | Partial | PR19/PR20 unapplied SQL and unmounted Drizzle transaction model provider-neutral plans/slots, exact micro-USD buckets, append-only approval/sandbox/quote/policy/kill-switch authority, locked governance, globally serialized count-based concurrency admission and atomic budget/reservation/slot CAS without provider side effects | Add authenticated authority issuers and production-scale concurrency counters, rehearse PostgreSQL contention/migration, then atomically create non-claimable jobs/outbox work only after every gate passes |
