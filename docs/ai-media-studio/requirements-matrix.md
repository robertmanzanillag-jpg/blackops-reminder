...
| Requirement | Status | Current evidence | Remaining acceptance |
| --- | --- | --- | --- |
| TikTok, Instagram, Facebook and YouTube Shorts | Partial | Provider-neutral contracts/ports, strict tenant-scoped durable account readiness, composite job/account/platform isolation, digest-only one-time OAuth sessions, mandatory PKCE S256, vault-reference contracts and legacy-unverified credential lifecycle cover all four platforms | Production vault, official per-platform manifests, authorization/code exchange, refresh/revocation, app review, adapters, ingestion and sandbox evidence per platform |
