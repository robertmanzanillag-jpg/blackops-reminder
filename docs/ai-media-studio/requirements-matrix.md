...
| Durable daily budget admission | Partial | PR19 pure domain, unapplied SQL and unmounted Drizzle transaction model provider-neutral plans/slots, exact micro-USD buckets, locked governance and atomic budget/reservation/slot CAS without provider side effects | Persist independent approval/sandbox/quote/policy/kill-switch authority, rehearse PostgreSQL contention/migration, then atomically create non-claimable jobs/outbox work only after every gate passes |
