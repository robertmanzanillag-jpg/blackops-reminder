...
| HeyGen adapters and terminal evidence | Partial | Legacy V2 remains mounted; PR27 adds an unmounted V3 admitted adapter, exact idempotent submit, authoritative GET by `video_id`, leased terminal polling, append-only account/credential/job evidence and atomic capacity release plus completed MP4 ingest handoff without false refund | Compose only the reviewed V3 path, add renewable artifact resolution, provider webhook-as-signal proof, staging migration/restart, one approved sandbox generation and billing/quota evidence |
