...
| Requirement | Status | Current evidence | Remaining acceptance |
| --- | --- | --- | --- |
| TikTok, Instagram, Facebook and YouTube Shorts | Partial | Provider-neutral contracts/ports, strict tenant-scoped durable account readiness, composite job/account/platform isolation, digest-only one-time OAuth sessions, mandatory PKCE S256, vault-reference contracts, legacy-unverified credential lifecycle, S3-KMS PKCE vault composition, and fixed provider authorization URL manifests cover all four platforms | Mounted start/callback routes, code exchange, long-lived token vaulting, refresh/revocation, app review, adapters, ingestion and sandbox evidence per platform |
