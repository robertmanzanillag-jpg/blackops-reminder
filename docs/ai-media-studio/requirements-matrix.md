...
| Database migration applied | Missing | PR30 inventories all 19 available forward/rollback pairs in a no-go, statically checked staging runbook; no database connection, migration or `db:push` has run. Provenance audit proves PR13 is schema-neutral and PR14 depends on PR12 DB controls; PR16 lacks reviewed SQL and complete relational invariants | Resolve PR1 provenance and PR16 schema/repository gates, merge the exact stack, identify staging, prove backup restore, provision separate least-privilege principals, obtain explicit approval, then run ordered forward/restart/reverse rehearsal with App QA as a hard deploy gate. Prove PR13 external storage readiness separately before restart/runtime/sandbox |
