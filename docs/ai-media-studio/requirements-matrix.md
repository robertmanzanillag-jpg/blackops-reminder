...
| One-video sandbox readiness packet and offline proof | Partial | The branch stacked on PR #146 adds an authenticated read-only exact plan/batch/slot packet with `9:16` preview, six local gates, five external requirements and every provider/spend/admission/effect flag false. Browser click coverage verifies explicit approved-slot selection. The disposable PostgreSQL 16 exact-22-chain harness uses a genuine approved 5 × 10 fixture and drives exactly one slot through fake submission, terminal completion, durable ingest, canonical asset and render linkage with zero publishing; focused 59/59, routes 4/4, browser 1/1, PostgreSQL 1/1, TypeScript/build/diff and checker/security/App QA P0–P3 pass | Create the GitHub draft; supply the HeyGen API key only through the secret manager plus 5–10 avatar/voice IDs; prove live account/credential connectivity, maximum quote, owned storage and callbacks, then obtain explicit one-video cost approval. The fake provider proves the local pipeline, not HeyGen or deployment. Require separate 5 × 10 canary spend and Replit deployment approvals |
