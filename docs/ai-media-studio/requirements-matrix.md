...
| Database migration applied | Missing | PR30 inventories all 19 available forward/rollback pairs in a no-go, statically checked staging runbook; no database connection, migration or `db:push` has run. SQL audit proves PR14 requires missing PR13 controls and PR16 also lacks reviewed SQL | Resolve PR1/PR13/PR16 provenance, merge the exact stack, identify staging, prove backup restore, provision separate least-privilege principals, obtain explicit approval, then run ordered forward/restart/reverse rehearsal with App QA as a hard deploy gate |
