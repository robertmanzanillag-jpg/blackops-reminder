...
| HeyGen Studio V2 adapter | Partial | V2 submit payload, strict `data.video_id`, canonical resource resolver and no ambiguous fallback are deny-by-default | Account-scoped status/reconciliation, sandbox generation, webhook and billing evidence with approved spend |
| Initial HeyGen avatar roster | Partial | Authenticated wizard and durable transaction accept 5–10 private avatar/voice mappings and plan 10 videos each without enqueueing | Provision one verified server account, complete rights/governance, cost admission, sandbox proof and launch approval |
| Daily roster plan preview | Partial | Server-timezone-bound, non-durable UI/API preview derives exactly 50–100 blocked, not-queued slots from the current roster with no generation/spend side effects | Persist plan/slots and bind source/script batches, governance proofs, atomic budget reservation, sandbox evidence and human launch approval before queueing |
| Durable daily budget admission | Partial | PR19–PR21 keep provider-neutral plans/slots, exact micro-USD buckets, capability-separated append-only approval/sandbox/quote/policy/kill-switch issuers, immutable snapshots, shared governance locking, globally serialized count-based admission and atomic budget/reservation/slot CAS unmounted and provider-free; an isolated PostgreSQL 16.14 run proves the exact PR19/PR20 SQL syntax, constraints and selected contention invariants | Compose trusted production RBAC and durable launch intent, provision buckets, add scale counters, rehearse a staging-copy migration/restart, then atomically create non-claimable jobs/outbox work only after every gate passes |
