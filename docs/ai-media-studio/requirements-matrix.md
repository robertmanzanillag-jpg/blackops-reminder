...
| Automatic generation directly from live Kong data | Partial | Draft PR #171 mounts bounded server-owned source synchronization; the current stacked slice adds an injected provider-neutral Kong-owned reader adapter for all eight categories, exact-content eligibility review, operator UI and deterministic preview for accepted owned/licensed and moderation-approved sources. These boundaries explicitly create no durable scripts, render/outbox or provider effects | Production Kong reader implementation, durable server-side cursor/scheduler, durable source-to-batch consumer and production rehearsal; generation remains behind the existing admission/cost gates |
