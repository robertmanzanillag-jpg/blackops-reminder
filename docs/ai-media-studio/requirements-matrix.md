...
Current delivery data is as of **2026-07-22**. Draft PRs #174 → #175 → #178 → #180 → #181 → #182 → #183 → #184 → #185 are review checkpoints, not merged or deployed releases. Checker/App QA evidence does not satisfy a human, migration, provider, spend, publishing or deployment gate.
...
| Videos published | Partial | PR #183 derives the dashboard total from confirmed publication records and excludes scheduled/queued work | Prove durable staging reconciliation against each live platform before treating it as an operational metric |
...
| Initial HeyGen avatar roster | Partial | Authenticated wizard and durable transaction accept 5–10 private avatar/voice mappings and plan exactly 10 videos each without enqueueing. PR #185 creates schema-valid provider-neutral starter personas and repairs invalid roster fields without overwriting valid editorial customization or reactivating archived creators | Robert places the API key only in the approved secret manager and supplies 5–10 names plus avatar-look/voice IDs; then rehearse PostgreSQL and separately approve GET-only verification, quote/cost admission and one video before any 5×10 canary or deployment. Starter personas are not rights/consent evidence |
...
| Dedicated AI Media Studio Agent | Partial | Authenticated read-only Kanban and Agents Office room expose Backlog, Review and Merged semantics, the #174→#185 chain, owners, evidence, blockers, next actions and explicit checker/App QA/CI/human gates; all effect flags stay false | Replace the dated static snapshot with durable GitHub/checker/App QA status only after independently reviewing the adapters; keep all mutation and launch authority server-side |
...
| Requirement | Status | Current evidence | Remaining acceptance |
| --- | --- | --- | --- |
| Trigger on new event, restaurant, hotel, promotion, deal or travel package | Partial | The current stacked slice adds an exact-host DNS-pinned reader for curated KONG feed PR #117, a tenant-scoped durable cursor with leased/fenced replay and a production 15-minute source-only scheduler. Exact curated rows are auto-attested and may prepare deterministic draft scripts after a complete cycle; browser/provider cursors and endpoint configuration remain rejected | Merge/review the stacked PRs, clear KONG baseline CI, rehearse/apply the existing DB chain with approval, deploy the KONG feed before the consumer and prove restart/recovery in staging |
...
| No-manual-work mode | Partial | Source ingestion, exact curated-source attestation and draft-script preparation are automated server-side. HeyGen generation, spend and publishing remain deliberately approval-gated | Prove deployed scheduler recovery; then separately approve credential verification, one-video sandbox, 5×10 spend policy and any publishing autonomy |
