...
PR12 adds the durable fenced callback saga and atomic credential-version/account binding without mounting it. PR13 adds inert authorization-code and long-lived token adapters with application-level KMS envelope encryption (AES-256-GCM) plus S3 SSE-KMS, immutable conditional writes, exact contextual AAD/KMS digests, deterministic recovery, strict absence/error handling and a separate token-secret reader capability. PR14 adds a dedicated durable cleanup outbox with fenced leases, exact source revalidation, two-pass deletion, conservative active-token retention, and an inert double-snapshot S3/KMS posture preflight. These components are exported but not composed into the runtime. The PR14 migration remains unapplied. Hard IAM separation, reviewed infrastructure/IAM evidence, provider target selection, live connectors, refresh/revocation and sandbox proof remain mandatory before any callback route.
