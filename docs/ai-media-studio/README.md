...
PR20 is the local `codex/ai-media-studio-durable-authorities` checkpoint stacked on PR #102. It replaces admission caller self-certification with append-only policy, kill-switch, launch-evidence, and exact launch-authority snapshot records. The reservation request now carries only tenant/plan/slot/bucket identity, an authority snapshot ID and digest, expected state versions, expiry, and idempotency evidence; provider identity, governance, approvals, sandbox result, quote amount, policy, kill switch, and provider idempotency are derived from locked database rows. A global admission advisory lock makes total/provider/tenant reservation counts race-safe in this inert scaffold; production-scale lease counters remain a later optimization. The migration and repository remain unapplied, unexported, and unmounted. Authenticated authority issuance and a live PostgreSQL contention/migration rehearsal are still required before runtime activation. This slice creates no job or outbox command, calls no provider, spends nothing, and deploys nothing.
