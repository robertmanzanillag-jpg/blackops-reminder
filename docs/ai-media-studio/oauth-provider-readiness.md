...
PR15 implements the first six items as provider-neutral domain and persistence contracts through the explicit-selection handoff (`activation_pending`). Meta exchange retains only the expiring grant-level User artifact descriptor; discovery persists no Page token. PR15 does not activate a provider account, store role-specific vault references, delete candidate tokens, mount a route, or call a provider.

PR16 is a local implementation in progress. It adds vault schema v2—one secret per artifact role, bound to the exact tenant/workspace/actor/account/platform/session/attempt/selection/target/credential-version context—and an exact activation/account CAS over the immutable PR15 selection. PR16A has local, unapplied forward/rollback SQL for candidate artifacts, exact bindings, v2 vault-operation evidence, lifecycle guards and deferred graph integrity, plus isolated PostgreSQL 16 proof. Local PR16B now implements PostgreSQL-owned selection digests, prewrite graph staging, exact durable finalization/replay, indeterminate recovery, separate fenced role-token-v2 cleanup and an AWS v2 posture preflight. Focused tests pass 14/14, isolated PostgreSQL tests pass 3/3, the full suite passes 688/0/34 of 722, TypeScript/build pass, and checker plus App QA are P0=P1=P2=0. This is still unmounted domain, repository, cleanup and preflight work: there is no route, activation orchestration worker, live provider exchange, real AWS operation or deployment. Refresh/revoke/reconciliation, effective IAM/IaC proof and provider sandbox evidence remain required before runtime wiring.
...
- PR16's role-specific vault v2 contract does not prove a configured bucket, CMK, role boundary or deletion lifecycle. The additive v2 operation records remain inert until cleanup composition and infrastructure gates are independently reviewed.
