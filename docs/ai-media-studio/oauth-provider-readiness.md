...
PR16 is a local implementation in progress. It adds the in-progress shape for vault schema v2—one secret per artifact role, bound to the exact tenant/workspace/actor/account/platform/session/attempt/selection/target/credential-version context—and an exact in-memory activation/account CAS that consumes the immutable PR15 selection. PR16A now has local, unapplied forward/rollback SQL for candidate artifacts, exact bindings, v2 vault-operation evidence, lifecycle guards and deferred graph integrity, plus isolated PostgreSQL 16 proof; independent checker review is pending. PR16B's executable durable activation repository and fenced CAS remain missing. This is domain, vault-adapter and schema work only: there is no mounted route, activation worker, live provider exchange, real AWS operation or deployment. The older callback saga and legacy publishing-readiness path are not upgraded by implication and remain fail-closed. Cleanup v2 runtime wiring, refresh/revoke/reconciliation, effective IAM/IaC proof and provider sandbox evidence remain required before runtime wiring.
