...
PR16 is a local, in-progress activation foundation rather than a runtime connector. It binds every activation to the immutable PR15 selection, exact discovered target, credential-version candidate and required artifact-role set. The account transition uses compare-and-swap evidence so stale, partial, ambiguous or target-mismatched activation attempts fail closed. Vault schema v2 stores one secret per artifact role under a role- and target-bound context instead of reusing the PR13 bundle reference. Additive schema declarations retain credential artifacts, the active account/credential binding and v2 vault-operation evidence; reviewed forward/rollback SQL and the executable Drizzle activation transaction remain pending. No route, worker loop, real AWS/provider operation or deploy is introduced, and the legacy publishing-readiness projection remains intentionally unusable until exact-target binding is integrated in a later slice.
