...
Status: connected MVP implemented for web chat, Telegram, GitHub handoff issues, App QA release gate, and Replit approval blocking.

## Goal

When Robert reports a bug, threat, security issue, or production problem from chat or Telegram, the assistant should route the work to Codex using the ChatGPT/Codex subscription, open a PR first, run App QA, report back, and only deploy to Replit after explicit approval.
...
6. Run the project checks plus App QA subagents.
7. If every QA gate passes, send Robert the PR summary.
8. Ask before Replit deploy.
9. Deploy only after Robert explicitly approves.
...
The app does not spend OpenAI API credits for Codex. It creates a GitHub handoff issue that Robert can open with Codex/Codex Cloud under the signed-in ChatGPT subscription workflow. The issue instructs Codex to work on a branch, open a PR, request `@codex review` on the PR, run checks, and leave Replit deployment blocked until Robert approves.
